...
class ChromeCastButton extends Button {
  ...
  initializeApi() {
    ...
    appId =
      this.options_.appId || chrome.cast.media.DEFAULT_MEDIA_RECEIVER_APP_ID;
    sessionRequest = new chrome.cast.SessionRequest(appId);
    apiConfig = new chrome.cast.ApiConfig(
      sessionRequest,
      ::this.sessionJoinedListener,
      ::this.receiverListener
    );
    return chrome.cast.initialize(
      apiConfig,
      ::this.onInitSuccess,
      ::this.castError
    );
  ...
  doLaunch() {
    ...
    if (this.apiInitialized) {
      return chrome.cast.requestSession(
  ...
  onSessionSuccess(session) {
    ...
    if (plTracks) {
      for (i = 0; i < plTracks.length; i++) {
        ...
        remotePlTrack =
          remotePlTracks &&
          remotePlTracks.trackElements_ &&
          remotePlTracks.trackElements_[i];
        trackId++;
        track = new chrome.cast.media.Track(
          trackId,
          chrome.cast.media.TrackType.TEXT
        );
        track.trackContentId = remotePlTrack
          ? remotePlTrack.src
          : "caption_" + plTrack.language;
      ...
      mediaInfo.textTrackStyle.windowType =
